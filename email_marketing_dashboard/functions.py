...
def generate_email_report(llm_client):
    ...
    email_performance_over_time = email_performance_over_time_response(
        llm_client, data['time_series']['delivery'][feature].rename(columns={'Daily': 'date'}))

    # Step 3: 图表生成
    email_domain_day_of_week = email_domain_day_of_week_response(
        llm_client, data['breakdowns']['delivery_by_domain'], data['breakdowns']['engagement_by_domain'],
...
def generate_social_media_report(llm_client):
